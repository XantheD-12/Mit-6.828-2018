...
page_init(void)
{
	...
	//extend memory alloced pages number
	size_t num_alloc=((uint32_t)boot_alloc(0)-KERNBASE)/PGSIZE;
	
	//IO hole alloced pages
	size_t num_hole=96;
...
page_alloc(int alloc_flags)
{
	...
	result->pp_link=NULL;
	//cprintf("page_alloc:%d\n",result->pp_ref);//delete
	...
	if(!(*pgdir_entry&PTE_P)){//PTE_P=0x001 //this page is not existed.
		if(!create){
			...
			*pgdir_entry = (page2pa(new_page) | PTE_P | PTE_W | PTE_U);
			//PTE_W-0x002,write; PRE_U-0x004,user can read
		}
		
	}	
	//PTE_ADDR()-((physaddr_t) (pte) & ~0xFFF)
	//PTX(va)-Page Table Index
...
page_lookup(pde_t *pgdir, void *va, pte_t **pte_store)
{
	...
	result=pa2page(PTE_ADDR(*pg_table_entry));
	return result;
}
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	for(;begin<end;begin+=PGSIZE){
		...
		//((uint32_t)(*page)&(perm|PTE_P))!=(perm|PTE_P)){
